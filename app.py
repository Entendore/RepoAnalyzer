...
class ProfileDialog(QDialog):
    def __init__(self, profile_manager, current_config, parent=None):
        ...
        btn_layout = QHBoxLayout()
        save_btn = QPushButton("💾 Save Current"); save_btn.clicked.connect(self.save_profile)
        load_btn = QPushButton("📂 Load"); load_btn.clicked.connect(self.load_profile)
        delete_btn = QPushButton("🗑️ Delete"); delete_btn.clicked.connect(self.delete_profile)
        btn_layout.addWidget(save_btn); btn_layout.addWidget(load_btn); btn_layout.addWidget(delete_btn)
        layout.addLayout(btn_layout)
        
        layout.addWidget(QLabel("<b>Built-in Presets:</b>"))
        presets_layout = QHBoxLayout()
        sec_audit = QPushButton("🔒 Security Audit"); sec_audit.clicked.connect(lambda: self.apply_preset("security_audit"))
        spring_clean = QPushButton("🧹 Spring Cleaning"); spring_clean.clicked.connect(lambda: self.apply_preset("spring_cleaning"))
        overview = QPushButton("📋 Quick Overview"); overview.clicked.connect(lambda: self.apply_preset("quick_overview"))
        presets_layout.addWidget(sec_audit); presets_layout.addWidget(spring_clean); presets_layout.addWidget(overview)
        layout.addLayout(presets_layout)

    def refresh_profiles(self): self.profile_list.clear(); self.profile_list.addItems(self.pm.list_profiles())
    def save_profile(self): 
        name = self.name_input.text().strip()
        if name: self.pm.save_profile(name, self.current_config()); self.refresh_profiles(); self.profile_list.setCurrentText(name)
    def load_profile(self): 
        name = self.profile_list.currentText()
        if name: self.selected_profile = self.pm.load_profile(name); self.accept()
    def delete_profile(self): 
        name = self.profile_list.currentText()
        if name: self.pm.delete_profile(name); self.refresh_profiles()
    def apply_preset(self, preset_name):
        presets = {
            "security_audit": {"vis_filter": "All", "status_filter": "Active", "type_filter": "Source", "stars_filter": 0, "stale_days": 90, "chk_health": False, "chk_popularity": False, "chk_license": True, "chk_ci": False, "chk_unprotected": True, "chk_stale": True, "chk_release": False},
            "spring_cleaning": {"vis_filter": "All", "status_filter": "All", "type_filter": "All", "stars_filter": 0, "stale_days": 180, "chk_health": True, "chk_popularity": True, "chk_license": False, "chk_ci": False, "chk_unprotected": False, "chk_stale": True, "chk_release": False},
            "quick_overview": {"vis_filter": "All", "status_filter": "Active", "type_filter": "Source", "stars_filter": 0, "stale_days": 365, "chk_health": False, "chk_popularity": True, "chk_license": False, "chk_ci": False, "chk_unprotected": False, "chk_stale": False, "chk_release": True}
        }
        self.selected_profile = presets.get(preset_name); 
        if self.selected_profile: self.accept()
...
class GithubWorker(QThread):
    ...
    def run(self):
        try:
            ...
            if self.flags.get('stale_days'): self.mark_stale(repos, self.flags['stale_days'])
    ...
    def run_gh(self, endpoint, silent_404=False):
        ...
        try:
            ...
            if silent_404 and "HTTP 404" in e.stderr: return None
    ...
    def enrich_repos(self, repos):
        needs_enrichment = any([self.flags.get('license'), self.flags.get('ci'), self.flags.get('unprotected'), self.flags.get('health'), self.flags.get('release')])
        if not needs_enrichment: return
        ...
        for i, repo in enumerate(repos):
            ...
            if self.flags.get('license'):
                ...
                if res: repo['license_det'] = res.get('license', {}).get('spdx_id', 'NONE') if res.get('license') else 'NONE'
            ...
            if self.flags.get('release'):
                res = self.run_gh(f"repos/{full_name}/releases/latest", silent_404=True)
...
class RepoExplorer(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.user_input = QLineEdit(); self.user_input.setPlaceholderText("Auto-detected if empty")
        ...
        self.vis_filter = QComboBox(); self.vis_filter.addItems(["All", "Public", "Private"])
        self.status_filter = QComboBox(); self.status_filter.addItems(["All", "Active", "Archived"])
        self.type_filter = QComboBox(); self.type_filter.addItems(["All", "Source", "Fork"])
        self.stars_filter = QSpinBox(); self.stars_filter.setRange(0, 1000000)
        self.stale_days = QSpinBox(); self.stale_days.setRange(1, 3650); self.stale_days.setValue(180)
        ...
        self.chk_use_cache = QCheckBox("Use Cache (24h TTL)"); self.chk_use_cache.setChecked(True)
        
        feat_layout.addWidget(self.chk_popularity); feat_layout.addWidget(self.chk_health)
        feat_layout.addWidget(self.chk_license); feat_layout.addWidget(self.chk_ci)
        feat_layout.addWidget(self.chk_unprotected); feat_layout.addWidget(self.chk_stale)
        feat_layout.addWidget(self.chk_release); feat_layout.addWidget(self.chk_use_cache)
        ...
        self.export_csv_btn = QPushButton("📁 Export CSV"); self.export_csv_btn.clicked.connect(lambda: self.export_data("csv"))
        self.export_json_btn = QPushButton("📄 Export JSON"); self.export_json_btn.clicked.connect(lambda: self.export_data("json"))
        self.export_md_btn = QPushButton("📝 Export Markdown"); self.export_md_btn.clicked.connect(lambda: self.export_data("markdown"))
        self.profile_btn = QPushButton("⚙️ Profiles / Presets"); self.profile_btn.clicked.connect(self.open_profile_manager)
        self.btn_clear_cache = QPushButton("🗑️ Clear Cache"); self.btn_clear_cache.clicked.connect(self.clear_cache)
        ...
        # Reports Tab
        self.reports_text = QTextEdit(); self.reports_text.setReadOnly(True)
        ...
        if not MATPLOTLIB_AVAILABLE:
            self.charts_layout.addWidget(QLabel("📊 Install matplotlib to enable charts: pip install matplotlib"))
    ...
    def apply_search(self, text):
        ...
        self.proxy_model.setFilterKeyColumn(2)  # Description column (searches name + desc)
    ...
    def open_repo_in_browser(self, index):
        ...
        if owner and repo: webbrowser.open(f"https://github.com/{owner}/{repo}")
    ...
    def show_context_menu(self, pos):
        ...
        if action == open_browser: webbrowser.open(f"https://github.com/{full_name}")
        elif action == copy_url: QApplication.clipboard().setText(f"https://github.com/{full_name}"); self.status_bar.showMessage("URL copied!", 2000)
        elif action == copy_name: QApplication.clipboard().setText(full_name); self.status_bar.showMessage("Name copied!", 2000)
        elif action == clone_ssh: subprocess.Popen(["git", "clone", f"git@github.com:{full_name}.git"]); self.status_bar.showMessage(f"Cloning {full_name} (SSH)...", 3000)
        elif action == clone_https: subprocess.Popen(["git", "clone", f"https://github.com/{full_name}.git"]); self.status_bar.showMessage(f"Cloning {full_name} (HTTPS)...", 3000)
        elif action == annotate_action: self.open_annotation_dialog(full_name)
    ...
    def clear_cache(self):
        self.cache.clear(); self.status_bar.showMessage("Cache cleared", 3000)
            ...
            if "Logged in" in result.stderr:
                for line in result.stderr.splitlines():
                    if "Logged in to github.com account" in line:
                        ...
                        self.status_bar.showMessage(f"✅ Authenticated as {uname}", 5000); return
            self.status_bar.showMessage("⚠️ Not authenticated. Run 'gh auth login'", 10000)
        except FileNotFoundError: QMessageBox.critical(self, "Error", "'gh' CLI not found. Please install it.")
    ...
    def get_current_config(self):
        return {
            "vis_filter": self.vis_filter.currentText(), "status_filter": self.status_filter.currentText(),
            "type_filter": self.type_filter.currentText(), "stars_filter": self.stars_filter.value(),
            "stale_days": self.stale_days.value(), "chk_health": self.chk_health.isChecked(),
            "chk_popularity": self.chk_popularity.isChecked(), "chk_license": self.chk_license.isChecked(),
            "chk_ci": self.chk_ci.isChecked(), "chk_unprotected": self.chk_unprotected.isChecked(),
            "chk_stale": self.chk_stale.isChecked(), "chk_release": self.chk_release.isChecked()
        }

    def apply_config(self, config):
        self.vis_filter.setCurrentText(config.get("vis_filter", "All")); self.status_filter.setCurrentText(config.get("status_filter", "All"))
        self.type_filter.setCurrentText(config.get("type_filter", "All")); self.stars_filter.setValue(config.get("stars_filter", 0))
        self.stale_days.setValue(config.get("stale_days", 180)); self.chk_health.setChecked(config.get("chk_health", False))
        self.chk_popularity.setChecked(config.get("chk_popularity", True)); self.chk_license.setChecked(config.get("chk_license", False))
        self.chk_ci.setChecked(config.get("chk_ci", False)); self.chk_unprotected.setChecked(config.get("chk_unprotected", False))
        self.chk_stale.setChecked(config.get("chk_stale", False)); self.chk_release.setChecked(config.get("chk_release", False))

    def start_fetch(self):
        self.fetch_btn.setEnabled(False); self.progress_bar.setVisible(True); self.progress_bar.setValue(0)
        self.previous_repos_data = dict(self.repos_data)
        self.repos_data.clear(); self.model.clear(); self.reports_text.clear()

        flags = {
            'health': self.chk_health.isChecked(), 'popularity': self.chk_popularity.isChecked(),
            'license': self.chk_license.isChecked(), 'ci': self.chk_ci.isChecked(),
            'unprotected': self.chk_unprotected.isChecked(), 'stale': self.chk_stale.isChecked(),
            'stale_days': self.stale_days.value(), 'release': self.chk_release.isChecked()
        }
        ...
        if user_raw:
            ...
            if user: targets.append((user, False))
        org_raw = self.org_input.text()
        if org_raw:
            org = extract_username(org_raw)
            if org: targets.append((org, True))

        if not targets:
            QMessageBox.warning(self, "Warning", "Please enter a Username or Organization.")
            self.fetch_btn.setEnabled(True); self.progress_bar.setVisible(False); return

        for owner, is_org in targets:
            if self.chk_use_cache.isChecked():
                cached = self.cache.get(owner, is_org, flags)
                if cached:
                    logger.info(f"Loaded {owner} from cache"); self.process_data(owner, cached)
                    self.update_progress(f"Loaded {owner} from cache", 100); continue
            
            worker = GithubWorker(owner, is_org, flags)
            worker.data_ready.connect(self.process_data); worker.progress.connect(self.update_progress)
            worker.error.connect(self.show_error); worker.finished.connect(self.worker_finished)
            self.workers.append(worker); worker.start()
        
        if not self.workers: self.worker_finished()

    def worker_finished(self):
        self.workers = [w for w in self.workers if w.isRunning()]
        if not self.workers:
            self.fetch_btn.setEnabled(True); self.progress_bar.setVisible(False)
            self.update_rate_limit(); self.generate_reports(); self.generate_charts()

    def update_progress(self, msg, pct):
        self.status_bar.showMessage(msg)
        if pct >= 0: self.progress_bar.setValue(pct)

    def show_error(self, msg): logger.error(msg); QMessageBox.critical(self, "Error", msg)

    def append_log(self, msg):
        self.log_text.append(msg)
        cursor = self.log_text.textCursor(); cursor.movePosition(QTextCursor.MoveOperation.End); self.log_text.setTextCursor(cursor)

    # ============================================
    # DATA PROCESSING & TABLE
    # ============================================
    def process_data(self, owner, repos):
        if self.chk_use_cache.isChecked(): self.cache.set(owner, False, {}, repos)  # Simplified cache key for direct calls
    ...
    def refresh_table(self):
        ...
        if self.chk_release.isChecked(): headers.extend(["Release", "Release Date"])
        
        self.model.setHorizontalHeaderLabels(headers)
        vis_f, status_f, type_f, stars_f = self.vis_filter.currentText().lower(), self.status_filter.currentText().lower(), self.type_filter.currentText().lower(), self.stars_filter.value()

        all_annotations = self.annotation_store.get_all()

        for owner, repos in self.repos_data.items():
            for r in repos:
                is_private = r.get('private', False); vis = "private" if is_private else "public"
                if vis_f != "all" and vis != vis_f: continue
                is_archived = r.get('archived', False); status = "archived" if is_archived else "active"
                if status_f != "all" and status != status_f: continue
                is_fork = r.get('fork', False); rtype = "fork" if is_fork else "source"
                ...
                def add_item(text, sort_val=None, color=None, tooltip=None):
                    ...
                    if sort_val is not None: item.setData(sort_val, Qt.UserRole)
                ...
                vis_color = QColor("red") if is_private else QColor("green"); add_item(vis.capitalize(), is_private, vis_color)
                stat_color = QColor("orange") if is_archived else QColor("green"); add_item(status.capitalize(), is_archived, stat_color)
                add_item(rtype.capitalize(), is_fork); add_item(r.get('language', '—'), r.get('language', ''))
                add_item(str(r.get('size', 0)), r.get('size', 0)); add_item(r.get('updated_at', '').split('T')[0], r.get('updated_at', ''))
                ...
                if self.chk_health.isChecked():
                    ...
                    lc = r.get('last_commit', ''); add_item(self.format_duration(lc) if lc else "N/A", lc)
                if self.chk_license.isChecked(): add_item(r.get('license_det', 'NONE'), r.get('license_det', 'NONE'))
                if self.chk_ci.isChecked():
                    has_ci = r.get('has_ci', False); add_item("✅" if has_ci else "❌", 1 if has_ci else 0)
                if self.chk_unprotected.isChecked():
                    is_prot = r.get('branch_protected', False); add_item("🛡️ Yes" if is_prot else "⚠️ No", 1 if is_prot else 0)
                if self.chk_stale.isChecked():
                    is_stale = r.get('is_stale', False); add_item("⚠️ Stale" if is_stale else "Fresh", 1 if is_stale else 0, QColor("magenta") if is_stale else None)
                if self.chk_release.isChecked():
                    add_item(r.get('latest_release', 'None'), r.get('latest_release', 'None'))
                    rd = r.get('release_date', ''); add_item(rd.split('T')[0] if rd else '—', rd)

                self.model.appendRow(row_items)

    def format_duration(self, date_str):
        try:
            past = datetime.fromisoformat(date_str.replace('Z', '+00:00')); now = datetime.now(timezone.utc); days = (now - past).days
            if days > 365: return f"{days // 365}y ago"; if days > 30: return f"{days // 30}mo ago"; return f"{days}d ago"
        except: return "N/A"
    ...
    def generate_charts(self):
        ...
        if not self.repos_data: return

        for owner, repos in self.repos_data.items():
            langs = {}
            for r in repos: lang = r.get('language') or 'None'; langs[lang] = langs.get(lang, 0) + 1
            if len(langs) > 8:
                sorted_langs = sorted(langs.items(), key=lambda x: x[1], reverse=True); top = dict(sorted_langs[:8]); top['Other'] = sum(v for _, v in sorted_langs[8:]); langs = top
            ...
            vis_data = {'Public': sum(1 for r in repos if not r.get('private')), 'Private': sum(1 for r in repos if r.get('private'))}
            self.charts_layout.addWidget(PieChartWidget(vis_data, f"Visibility — {owner}"))
            
            if self.chk_popularity.isChecked():
                star_data = {r.get('name', ''): r.get('stargazers_count', 0) for r in sorted(repos, key=lambda x: x.get('stargazers_count', 0), reverse=True)[:10]}
            ...
            for owner in self.repos_data:
                if owner in self.previous_repos_data:
                    ...
                    if diff['added'] or diff['removed'] or diff['changed']:
                        ...
                        if diff['added']: report += "<h3 style='color:#3fb950'>🆕 New Repos</h3><ul>" + "".join(f"<li>{n}</li>" for n in diff['added']) + "</ul>"
                        if diff['removed']: report += "<h3 style='color:#f85149'>🗑️ Removed Repos</h3><ul>" + "".join(f"<li>{n}</li>" for n in diff['removed']) + "</ul>"
                        if diff['changed']:
                            report += "<h3 style='color:#d29922'>📝 Changed</h3><ul>"
                            for item in diff['changed'][:20]: report += f"<li><b>{item['repo']}</b><ul>" + "".join(f"<li>{f}: {v['from']} → {v['to']}</li>" for f, v in item['changes'].items()) + "</ul></li>"
                            report += "</ul>"

        for owner, repos in self.repos_data.items():
            report += f"<h2>Summary for {owner}</h2>"
            total, public, active = len(repos), sum(1 for r in repos if not r.get('private')), sum(1 for r in repos if not r.get('archived'))
            ...
            langs = {}
            for r in repos: lang = r.get('language', 'None') or 'None'; langs[lang] = langs.get(lang, 0) + 1
            report += "<h3>Languages</h3><ul>" + "".join(f"<li>{k}: {v}</li>" for k, v in sorted(langs.items(), key=lambda item: item[1], reverse=True)) + "</ul>"
            ...
            if self.chk_stale.isChecked():
                ...
                report += "".join(f"<li>{r.get('full_name')} (Last: {r.get('updated_at', '').split('T')[0]})</li>" for r in stale_repos) or "<li>No stale repos!</li>"
                report += "</ul>"
            ...
            if self.chk_unprotected.isChecked():
                ...
                report += "".join(f"<li>{r.get('full_name')} ({r.get('checked_branch', 'main')})</li>" for r in unprot) or "<li>All protected!</li>"
                report += "</ul>"

        self.reports_text.setHtml(report)

    def compute_diff(self, old_repos, new_repos):
        old_map, new_map = {r['full_name']: r for r in old_repos}, {r['full_name']: r for r in new_repos}
        old_names, new_names = set(old_map.keys()), set(new_map.keys())
        changed = []
        for name in old_names & new_names:
            old_r, new_r, changes = old_map[name], new_map[name], {}
            for field in ['stargazers_count', 'forks_count', 'open_issues_count', 'private', 'archived', 'language', 'size', 'updated_at']:
                if old_r.get(field) != new_r.get(field): changes[field] = {'from': old_r.get(field), 'to': new_r.get(field)}
            if changes: changed.append({'repo': name, 'changes': changes})
        return {'added': sorted(new_names - old_names), 'removed': sorted(old_names - new_names), 'changed': changed}

    # ============================================
    # EXPORTS
    # ============================================
    def export_data(self, fmt):
        if not self.repos_data: QMessageBox.warning(self, "No Data", "Fetch repositories first."); return
        ...
        if fmt == "csv":
            ...
            if path:
                with open(path, 'w', newline='', encoding='utf-8') as f:
                    ...
                    for r in range(self.proxy_model.rowCount()): writer.writerow([str(self.proxy_model.data(self.proxy_model.index(r, c))) for c in range(self.proxy_model.columnCount())])
                QMessageBox.information(self, "Exported", f"Saved to {path}")

        elif fmt == "json":
            path, _ = QFileDialog.getSaveFileName(self, "Save JSON", "repos.json", "JSON Files (*.json)")
            if path:
                with open(path, 'w', encoding='utf-8') as f: json.dump(self.repos_data, f, indent=2, ensure_ascii=False)
                QMessageBox.information(self, "Exported", f"Saved to {path}")
            ...
            if path:
                with open(path, 'w', encoding='utf-8') as f:
                    ...
                    for owner, repos in self.repos_data.items():
                        ...
                        for r in repos:
                            vis = "🔒" if r.get('private') else "🌐"; desc = (r.get('description') or '').replace('|', '\\|')
